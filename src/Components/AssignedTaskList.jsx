...
const AssignTaskList = ({editingMode,positivePage,cust_data,fetchCustomer,isLoadingState}) => {
  ...
  const deleteData = async (id)=>{
    await delete_item({id,table:"tasks"})
    fetchCustomer()
  }
  ...
  const updateState =async (value,id)=>{
    ...
    await update_item({data,table:"tasks",id})
    setTimeout(() => {
        if(positivePage){
            fetchCustomer()
        }else{
        ...
        <div className='relative'>
            ...
            }
            {
                !isLoadingState&&cust_data?.length === 0 && (
                        <p colSpan="8" className="text-center absolute top-10 z-30 left-0 right-0 m-auto py-4">
                        No Data
                        </p>
                ) 
            }
        ...
        <table className="min-w-full min-h-14 bg-white dark:bg-gray-800 shadow-md rounded-md">
          <thead>
            <tr className='text-left'>
              ...
              <th className="py-2 px-4 border-b text-sm">Assigned To</th>
              <th className="py-2 px-4 border-b text-sm">At</th>
              <th className="py-2 px-4 border-b text-sm">By</th>
            ...
            {(
                ...
                <tr key={index} className='text-left'>
                  ...
                  <td className="py-2 px-4 border-b text-sm">
                    <p>{customer.name}</p>
                    <p className='text-xs text-gray-400'>{customer?.emplyee?.designation}</p>
                  </td>
                  <td className="py-2 px-4 border-b text-sm">
                    <p>{moment(customer?.created_at).format("lll")}</p>
                    <p className='text-gray-400'>{moment(customer?.created_at).fromNow()}</p>
                  </td>
                  <td className="py-2 px-4 border-b text-sm">
                    <p>{customer.assigned_by?.name}</p>
                    <p>{customer.assigned_by?.email}</p>
                  </td>
                  ...
                  <td className="py-2 px-4 border-b text-sm space-x-2">
                    <span className='cursor-pointer' onClick={()=>setselecteddata({Task:customer?.task_details})}><FontAwesomeIcon icon={faEye} /></span>

                    {instancedata?.login_info?.role!=='Employee'&&<span onClick={()=>editingMode(customer)} className='cursor-pointer'><FontAwesomeIcon icon={faEdit} /></span>}

                    {instancedata?.login_info?.role!=='Employee'&&<span onClick={()=>deleteData(customer?._id)}><FontAwesomeIcon icon={faTrash} /></span>}
                  </td>
                </tr>
              ))
            )}
