...
const Meetings = () => {
  ...
  const createScedule = ()=>{
    ...
    try {
      setisLoading(true)
      const message = "Your meeting will start in 10 minutes with "+formData?.customerName+"."
      let data = {
  ...
  const handleSubmit = async (e) => {
    ...
    if(isEditingMode && editId){
        ...
        await update_item({data:{...formData,numbers:numarr,meeting:"yes"},table:'meetings',id:editId})
    ...
    const numarr =typeof formData?.numbers==='string'?JSON.stringify(formData?.numbers?.split(',')):formData?.numbers;
    await add_item({data:{...formData,numbers:numarr,file:uploadedFileLink,meeting:"yes"},table:'meetings'})
    createScedule()
    setisAdding(false)
  ...
  const fetchCustomer = (data={})=>{
    ...
    fetch(apilink+"/db/get-item",{
        ...
        body:JSON.stringify({query:{...data,meeting:'yes'},table:"meetings"})
    })
    .then((res)=>res.json())
    .then((result)=>{
        setisLoading(false)
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className='flex justify-between mb-4 items-center border-b py-2'>
        <h1 className="text-2xl font-bold">Meetings Form</h1>
        {instancedata?.login_info?.role!=='Employee'&&<p className='cursor-pointer px-2 text-white py-1 bg-blue-500 rounded-md' onClick={()=>setseeForm((prev)=>!prev)}><FontAwesomeIcon icon={faPlus} /> Add Meetings</p>}
      </div>
      ...
      {/* List of Customers */}
      {<MeetingsList cust_data={customerdata} editingMode={editingMode} fetchCustomer={fetchCustomer} isEditingMode={isEditingMode} isLoadingState={isLoading}/>}
    </div>
  );
};
