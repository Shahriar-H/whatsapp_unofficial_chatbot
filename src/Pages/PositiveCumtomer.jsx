...
const PositiveCustomer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(isEditingMode && editId){
        ...
        delete formData['_id']
        await update_item({data:{...formData},table:'customers',id:editId})
    ...
    await add_item({data:{...formData,positive:"No"},table:'customers'})
    setisAdding(false)
    setFormData({
      firstname: '',
      lastname:"",
  ...
  const fetchCustomer = ()=>{
    ...
    .then((result)=>{
        ...
        const ewr = result?.result.filter((item)=>item?.positive==='Yes'&&item)
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className='flex justify-between mb-4 items-center border-b py-2'>
        <h1 className="text-2xl font-bold">Positive Customer</h1>
      ...
      {/* Form */}
      {seeForm&&<form onSubmit={handleSubmit} className="bg-white p-4 rounded-md shadow-md">
      ...
      {/* List of Customers */}
      {<CompleteCustomerList cust_data={customerdata} editingMode={editingMode} fetchCustomerpositive={fetchCustomer} isEditingMode={isEditingMode} isLoadingState={isLoading} positivePage={true}/>}
    </div>
  );
};

export default PositiveCustomer;
